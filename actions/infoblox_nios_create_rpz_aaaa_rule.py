# File: infoblox_nios_create_rpz_aaaa_rule.py
#
...
class CreateRpzAAAARuleAction(BaseAction):
    """Create RPZ AAAA Rule action class."""

    def __log_action_start(self):
        """Log the start of the action execution."""
        self._connector.save_progress("Starting Create RPZ AAAA Rule action")
    ...
    def __get_endpoint_and_prepare_data(self):
        ...
        if object_type == "Domain Name":
            endpoint = f"{consts.RPZ_AAAA_DOMAIN_ENDPOINT}?{consts.RETURN_FIELDS_PARAM_PRZ_AAAA}"
        elif object_type == "IP address":
            endpoint = f"{consts.RPZ_AAAA_IP_ENDPOINT}?{consts.RETURN_FIELDS_PARAM_PRZ_AAAA}"
        else:
            return None, None
        ...
        data = {
            "name": formatted_name,
            "rp_zone": self._param["rp_zone"],
            "ipv6addr": self._param["ipv6_address"],
        }
    ...
    def __make_api_call(self):
        """Make the REST API call to create the RPZ AAAA rule.
    ...
    def __handle_response(self, response):
        """Handle the API response for create RPZ AAAA rule.
        ...
        message = consts.SUCCESS_CREATE_RPZ_AAAA_RULE
        if ref_id:
    ...
    def execute(self):
        """Execute the Create RPZ AAAA Rule action.

        Step 1: Log action start
        Step 2: Validate parameters
        Step 3: Make API call to create RPZ AAAA rule
        ...
        if phantom.is_fail(status):
            ...
            return self._action_result.set_status(phantom.APP_ERROR, f"{consts.ERROR_CREATE_RPZ_AAAA_RULE}: {error_msg}")
