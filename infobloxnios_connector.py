# File: infobloxnios_connector.py
#
...
from infobloxnios_consts import (
    INFOBLOXNIOS_CONNECTIVITY_ENDPOINT,
    INFOBLOXNIOS_ERR_CONNECTIVITY_TEST,
    INFOBLOXNIOS_SUCC_CONNECTIVITY_TEST,
)


class InfobloxniosConnector(BaseConnector):
    """
    Infobloxnios connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = INFOBLOXNIOS_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(INFOBLOXNIOS_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(INFOBLOXNIOS_SUCC_CONNECTIVITY_TEST)
